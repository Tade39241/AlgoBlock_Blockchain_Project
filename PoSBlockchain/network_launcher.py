...
class NetworkLauncher:
    ...
    #     """Update NodeDB to include all nodes in the network"""
    #     try:
    ...
    #         # Initialize databases with custom paths for each node
    #         node_ports = []
    ...
    #                 # For node.db specifically, create and initialize it directly with SQL
    #                 if db_name == "node.db":
    #                     conn = sqlite3.connect(db_path)
    #                     cursor = conn.cursor()
    #                     # Drop table if exists to clear it
    #                     cursor.execute('DROP TABLE IF EXISTS nodes')
    #                     # Create nodes table
    #                     cursor.execute('CREATE TABLE IF NOT EXISTS nodes (port INTEGER PRIMARY KEY)')
    #                     # Add this node's port
    #                     cursor.execute('INSERT INTO nodes VALUES (?)', (port,))
    #                     conn.commit()
    #                     conn.close()
    #                 else:
    #                     # For other databases, just create empty files
    #                     conn = sqlite3.connect(db_path)
    #                     conn.close()
                    
    #                 print(f"Created database: {db_path}")
                
    #             print(f"Initialized databases for node {node_info['id']}")
            
    #         # Add all other nodes to each node's database
    #         for node_info in self.node_info:
    #             node_dir = node_info["dir"]
    #             node_db_path = os.path.join(node_dir, "node.db")
    #             conn = sqlite3.connect(node_db_path)
    #             cursor = conn.cursor()
                
    #             # Add all other nodes
    #             for port in node_ports:
    #                 if port != node_info["port"]:  # Don't add itself twice
    #                     try:
    #                         cursor.execute('INSERT OR IGNORE INTO nodes VALUES (?)', (port,))
    #                     except sqlite3.IntegrityError:
    #                         # Port already exists, ignore
    #                         pass
                
    #             conn.commit()
    #             conn.close()
                
    #         print(f"Updated NodeDBs with {self.num_nodes} nodes")
            
    #     except Exception as e:
    #         print(f"Error in update_node_db: {e}")
    #         import traceback
    #         traceback.print_exc()
    #         raise

    # filepath: /Users/tadeatobatele/Documents/UniStuff/CS351 Project/code/PoSBlockchain/network_launcher.py
    ...
    #     # Create validator database files
    #     import sqlite3
    #     for db_name in ["node.db", "blockchain.db", "account.db"]:
    ...
    #     # Create peer list from all nodes
    #     peer_list = [(node["host"], node["port"]) for node in self.node_info]
        
    #     print(f"Starting validator node on port {validator_port} with peers: {peer_list}")
        
    #     # Set environment variables for validator to use custom paths
    #     env = os.environ.copy()
    #     env["VALIDATOR_DB_DIR"] = data_dir
        
    #     # Create validator script
    #     self.create_validator_script(validator_dir, validator_port, peer_list)
        
    #     # Start validator process with the script
    #     validator_script = os.path.join(validator_dir, "run_validator.py")
    ...
    def transaction_loop():
        ...
        while True:
            ...
            try:
                ...
                print(f"[Sim Debug {os.getpid()}/{{threading.get_ident()}}] Calling db_instance.read()...")
                accounts = db_instance.read() # Use the instance we just checked
                print(f"[Sim Debug {os.getpid()}/{{threading.get_ident()}}] db_instance.read() completed.")
