...
def miner_entry(utxos, mem_pool, newBlockAvailable, secondaryChain,localHostPort, localHost, node_id, db_path,my_public_addr):
            ...
            print(f"[miner_entry 0] Performing initial sync…", flush=True)
            try:
                bc.startSync()
                print(f"[miner_entry 0] Sync complete, starting miner", flush=True)
            except Exception as e:
                print(f"[miner_entry 0] Sync failed: {e}", flush=True)
            bc.main()
