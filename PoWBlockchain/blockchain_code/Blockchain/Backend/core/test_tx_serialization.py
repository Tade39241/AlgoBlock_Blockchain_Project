...
def test_tx_id_consistency():
    ...
    tx = Tx(1, [tx_in], [tx_out], 0)
    assert tx.id() == tx.hash().hex()

from unittest.mock import Mock

def test_broadcast_and_receive():
    tx = Mock()  # Mock a transaction
    tx.id.return_value = "mock_tx_id"
    tx.serialise.return_value = b"mock_serialized_tx"
    ...
    # Simulate receiving
    received_tx = Tx.parse(tx.serialise())
